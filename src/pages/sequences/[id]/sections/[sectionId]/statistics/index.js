...
const Statistics = () => {
    ...
    const { getSequenceGroupStats } = useUser({
        middleware: 'auth',
    })

     useEffect(() => {

        (slug && sectionId) && getSequenceGroupStats({ slug, sectionId, setStatistics, setPending })
                                        ...
                                        {statistics?.map((statistic, index) => (
                                            statistic.data && (
                                                <Statistic
                                                    ...
                                                    link={statistic.data.min ? `/sequences/${slug}/classrooms/${statistic.id}/statistics`: 'Ras'}
                                                />
                                            )
                                        ))}
